  ...
  return (
    <section id="stack" className="py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="overflow-hidden w-full py-6 mb-12 relative before:absolute before:left-0 before:top-0 before:z-10 before:h-full before:w-20 before:bg-gradient-to-r before:from-[#030712] before:to-transparent after:absolute after:right-0 after:top-0 after:z-10 after:h-full after:w-20 after:bg-gradient-to-l after:from-[#030712] after:to-transparent">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, catIdx) => (
            <div key={catIdx} className="glass p-6 rounded-2xl border border-white/5 relative overflow-hidden group">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4">
                {category.skills.map((item, idx) => (
                  <div 
                    key={idx} 
                    className={`glass p-4 rounded-xl flex items-center gap-3 transition-all duration-300 border border-white/5 ${item.glowClass}`}
                  >
                    <span className={`text-3xl ${item.color} transition-transform duration-300`}>{item.icon}</span>
